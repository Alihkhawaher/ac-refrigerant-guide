...
function renderDiagram() {
  ...
  var compData = {
    EVAPORATOR: {title:isAr?'مبخر':'Evaporator',lines:isAr?['الضغط: '+rg.loMin+'-'+rg.loMax+' PSIG','حرارة: ~'+evapT+'°C','هدف السوبريتي: '+r.superheatTarget]:['Pressure: '+rg.loMin+'-'+rg.loMax+' PSIG','Temp: ~'+evapT+'°C','Superheat target: '+r.superheatTarget]},
    COMPRESSOR: {title:isAr?'ضاغط':'Compressor',lines:isAr?['شفط: '+rg.loMin+'-'+rg.loMax+' PSIG','تفريغ: '+rg.hiMin+'-'+rg.hiMax+' PSIG','نوع الزيت: '+T(r.oil)]:['Suction: '+rg.loMin+'-'+rg.loMax+' PSIG','Discharge: '+rg.hiMin+'-'+rg.hiMax+' PSIG','Oil type: '+T(r.oil)]},
    CONDENSER: {title:isAr?'مكثف':'Condenser',lines:isAr?['الضغط: '+rg.hiMin+'-'+rg.hiMax+' PSIG','حرارة: ~'+condT+'°C','هدف التبريد: '+r.subcoolingTarget]:['Pressure: '+rg.hiMin+'-'+rg.hiMax+' PSIG','Temp: ~'+condT+'°C','Subcooling target: '+r.subcoolingTarget]},
    TXV: {title:isAr?'صمام تمديد':'TXV',lines:isAr?['انخفاض الضغط: '+rg.hiMin+' > '+rg.loMin+' PSIG','يتحكم في السوبريتي']:['Pressure drop: '+rg.hiMin+' > '+rg.loMin+' PSIG','Controls superheat']},
    EVAPFAN: {title:isAr?'مروحة مبخر':'Evap Fan',lines:isAr?['يدفع الهواء عبر المبخر']:['Forces air across evaporator']},
    CONDFAN: {title:isAr?'مروحة مكثف':'Cond Fan',lines:isAr?['تبرد المكثف']:['Cools the condenser']}
  };
  _diagCompData = compData;
  var _pos = window._diagPos;
  var W = 720, H = 530;
  var evap = {x:_pos.evap.x,y:_pos.evap.y,w:180,h:110};
  var evapFan = {x:_pos.evapFan.x,y:_pos.evapFan.y,w:80,h:45};
  var comp = {x:_pos.comp.x,y:_pos.comp.y,w:140,h:90};
  var cond = {x:_pos.cond.x,y:_pos.cond.y,w:180,h:110};
  var condFan = {x:_pos.condFan.x,y:_pos.condFan.y,w:80,h:45};
  ...
  svg += '<path d="M'+sX1+' '+sY1+' L'+(wallX-5)+' '+sY1+' L'+(wallX-5)+' '+(sY2-30)+' L'+(wallX+wallW+10)+' '+(sY2-30)+' L'+(wallX+wallW+10)+' '+sY2+' L'+sX2+' '+sY2+'" fill="none" stroke="#00b4d8" stroke-width="3" marker-end="url(#arrowB)"/>';
  ...
  svg += box(evapFan.x,evapFan.y,evapFan.w,evapFan.h,'#0a1a2a','#4488aa','EVAPFAN');
  svg += box(evap.x,evap.y,evap.w,evap.h,'#0d2137','#00b4d8','EVAPORATOR');
  svg += box(comp.x,comp.y,comp.w,comp.h,'#2a1a1a','#e63946','COMPRESSOR');
  svg += box(cond.x,cond.y,cond.w,cond.h,'#2a1a0a','#f4a261','CONDENSER');
  svg += box(condFan.x,condFan.y,condFan.w,condFan.h,'#2a1a0a','#aa6633','CONDFAN');
  ...
  svg += txt(comp.x+comp.w/2,comp.y+28,isAr?'ضاغط':'COMPRESSOR',12,'#e63946');
  ...
  svg += txt(txv.x+txv.w/2,txv.y+22,'TXV',12,'#2a9d8f');
  svg += '</svg>';
...
function toggleLang() {
  ...
  var tabNames = {en:{instructions:'📋 Instructions',ptchart:'📊 PT Chart',calculators:'🔧 Calculators',target:'🎯 Target Pressures',compare:'📊 Compare',diagram:'📐 System Diagram',issues:'⚠️ Issues',systemcalc:'🔧 System Calc',maintenance:'🧹 Maintenance',commonknowledge:'📖 Common Knowledge'},ar:{instructions:'📋 التعليمات',ptchart:'📊 مخطط الضغط-الحرارة',calculators:'🔧 الحاسبات',target:'🎯 الضغوط المستهدفة',compare:'📊 مقارنة',diagram:'📐 مخطط النظام',issues:'⚠️ المشاكل',systemcalc:'🔧 حاسبة النظام',maintenance:'🧹 الصيانة',commonknowledge:'📖 معلومات عامة'}};
